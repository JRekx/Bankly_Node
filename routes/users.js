...
router.get('/', authUser, requireLogin, async function(req, res, next) {
  try {
    ...
    // Map users to only include basic info
    users = users.map(u => ({
      username: u.username,
...
router.patch('/:username', authUser, requireLogin, async function(
  ...
  try {
    ...
    // Validate fields to change
    const allowedFields = ['first_name', 'last_name', 'phone', 'email'];
    for (let field in req.body) {
      if (!allowedFields.includes(field)) {
        throw new ExpressError(`You cannot update field: ${field}`, 400);
      }
    }
...
router.delete('/:username', authUser, requireAdmin, async function(
  ...
  try {
    await User.delete(req.params.username); // Use await to handle the promise
